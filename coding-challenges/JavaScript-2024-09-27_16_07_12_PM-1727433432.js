/* Your task is to write a function which formats a duration, given as a number of seconds, in a
human-friendly way.
...
function convertSecToHumanFriendly(sec) {
  let time = '';
  let years, days, hours, minutes, seconds;
  let secNumeric = parseInt(sec, 10);

  if (secNumeric < 0) {
    return 'Please add valid sec';
  }
  if (secNumeric === 0) {
    return 'Now';
  }

  years = Math.floor(secNumeric / (365 * 24 * 60 * 60));
  secNumeric %= 365 * 24 * 60 * 60;

  days = Math.floor(secNumeric / (24 * 60 * 60));
  secNumeric %= 24 * 60 * 60;

  hours = Math.floor(secNumeric / (60 * 60));
  secNumeric %= 60 * 60;

  minutes = Math.floor(secNumeric / 60);

  seconds = secNumeric % 60;

  if (years > 0) time += `${years} year${years > 1 ? 's' : ''} `;
  if (days > 0) time += `${days} day${days > 1 ? 's' : ''} `;
  if (hours > 0) time += `${hours} hour${hours > 1 ? 's' : ''} `;
  if (minutes > 0) time += `${minutes} minutes${minutes > 1 ? 's' : ''} `;
...
const sec = 60;
console.log(convertSecToHumanFriendly(sec));
